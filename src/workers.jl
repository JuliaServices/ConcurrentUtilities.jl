...
terminated(w::Worker) = w.terminated
...
function Worker(;
    ...
    cmd = `$(Base.julia_cmd()) $exeflags --startup-file=no --color=$color -e 'using ConcurrentUtilities: Workers; Workers.startworker()'`
    proc = open(detach(setenv(addenv(cmd, env), dir=dir)), "r+")
    pid = Libc.getpid(proc)

    ## connect to the worker process with timeout
    try
        sock = try_with_timeout(connect_timeout) do timedout
            port_str = ""
            while !timedout[] && isempty(port_str) && !contains(port_str, r"juliaworker:\d+")
